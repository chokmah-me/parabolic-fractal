...
**Cohort A (baseline, human-written):** 15 mature Python OSS projects, all with >5 years of development and >100 contributors: Django, Flask, NumPy, pandas, SQLAlchemy, Celery, FastAPI, requests, pytest, Scrapy, httpx, Pydantic, aiohttp, Tornado, Click. Note that FastAPI the *framework* (Cohort A) has rich internal coupling (Gini=0.981); FastAPI-based *applications* (several in Cohort B) inherit the framework's design philosophy of thin routers but not its internal structure. **Post-release validity note (2026-09-22):** a full-history audit for declared AI-attribution commits (Section 7.7) found 10 of these 15 repos contain a minority of individually AI-attributed commits (e.g. pandas: 1,067/39,081; pytest: 132/17,781; full counts in `data/contamination-baseline.csv`). Repo-level "baseline" reflects project-level selection criteria (mainstream, human-founded, pre-dating the agentic-coding era), not commit-level purity; see Section 7.7 for the resulting robustness check.
...
The cross-sectional result cannot rule out a simpler explanation: that AI-generated and human-written repos differ on confounding variables (age, size, contributors, domain). A within-repo longitudinal design removes these confounds in principle. If the same codebase develops lower Gini and weaker log-normal signal after adopting AI coding tools, the change is attributable to the process, not to repo identity. We report this as a pilot, not a validation study: with N=2 mature repos and a 12-month post-adoption window, the design is exploratory and cannot rule out continued natural maturation as the source of any observed trend. **This within-repo design is the primary evidence for the AI-topology question in this paper** (Section 7.7): unlike the cross-sectional Cohort A/B split, it is immune to the baseline-contamination issue discovered post-release, since each repo is compared only against its own earlier history.

### 5.1 Repo Selection and Declared-Attribution Dates

`find_adoption_date.py` detects the earliest **declared** AI attribution in a repo's commit history or config files — a literal marker such as a `Co-authored-by: ... noreply@anthropic.com` trailer or the appearance of `CLAUDE.md` — not tool adoption itself. Silent (undeclared) adoption is not detectable by this or any commit-message-based method; see Section 7.7 for the validation evidence behind that claim. We scanned the 15 baseline repos for these signals. Three confirmed candidates yielded dates:

- **Celery**: first Copilot co-author commit 2025-05-08 (corrected to 2025-05-09 after a parser fix in Section 7.7; a 1-day shift that does not change any monthly snapshot boundary). Config file `.github/copilot-instructions.md` appeared 2025-08-26. Earliest signal: 2025-05-08/09.
- **Django**: `.github/copilot-instructions.md` added 2026-03-05. No commit-level AI attribution. Declared-attribution date: 2026-03-05.
- **aiohttp**: Claude Opus co-authored commit 2026-05-04; `CLAUDE.md` appeared 2026-05-17. Excluded: only 15 days of post-attribution data at collection time. (A later full-history audit found aiohttp also has 3 attributed commits going back to 2026-05-16 — consistent with, not contradicting, this date.)

Inclusion criteria: Python primary language, created before 2023 (ensuring at least 24 months of pre-attribution history), full git history available, >50 Python files at the declared-attribution date, at least one unambiguous signal. Both Celery and Django have full 24-month pre-attribution windows.
...
### 7.7 Post-Release Validity Audit (2026-09-22)

Two issues were identified after initial release, addressed here rather than by silently revising the results above.

**Baseline contamination.** A repo-level "baseline = human-written" label does not guarantee commit-level purity. A full-history scan for declared AI-attribution commits (same detector as `find_adoption_date.py`, reused unchanged in `scripts/longitudinal/audit_contamination.py`, run against all 15 baseline repos after unshallowing) found 10 of 15 contain at least one such commit: pandas (1,067/39,081 commits), pytest (132/17,781), tornado (33/5,133), numpy (24/42,167), scrapy (5/11,511), pydantic (4/5,746), celery (9/13,332, consistent with its already-known declared-attribution date), fastapi (3/7,713), aiohttp (3/14,164), and requests (1/6,495). Django, Flask, httpx, Click, and SQLAlchemy showed none. Full counts: `data/contamination-baseline.csv`; per-repo clone provenance for all 37 repos (baseline + agentic): `data/repos/provenance.csv`.

This does not retract the cross-sectional finding (Section 4), but it demotes it: the Cohort A/B comparison is a **secondary, contamination-flagged result**, while the **within-repo longitudinal design (Sections 5-6) is the primary evidence**, since it is immune to this confound by construction. Re-running the Mann-Whitney comparison with the 10 contaminated repos excluded (n=5 baseline; `data/compare_groups_excl_contaminated.csv`, vs. the full-15 result in `data/compare_groups_with_contaminated.csv`) preserves the same direction on Gini (p=0.015, r=-0.767 vs. the full-sample p=0.0011, r=-0.744) and normalized entropy (p=0.035 vs. p=0.002), but loses significance on delta-AIC and fraction-leaf-files at n=5 — an expected consequence of the smaller sample, not evidence against the direction of the effect.

**Parser bug in `earliest_commit_signal()`.** While re-deriving dates for this audit, we found the original commit-scanning function only tested each commit's *first output line* against the attribution regex (a `%ai %s %b` git-log format flattened via naive `split(" ", 3)` + `splitlines()`), so any `Co-authored-by:` trailer placed later in a multi-line commit body — the normal position for such trailers — was silently invisible to it. This was fixed to use NUL-safe field/record separators (matching the approach already used in `audit_contamination.py`). Re-running on full history: Celery's declared-attribution date moves from 2025-05-08 to 2025-05-09 (the bug had missed an earlier commit, not fabricated a false one); Django's is unchanged (its date came from a config-file signal, unaffected by this bug). A 1-day shift changes no monthly snapshot boundary in `walk_history.py`, so Sections 5-6's results are unaffected — we report this as a confirmed non-movement, not an unverified assumption. The bug is corrected in the shipped script for future runs.

**Construct: declared attribution, not adoption.** `find_adoption_date.py` and this audit both detect *declared* AI attribution — a literal commit-message trailer or config-file marker — not tool adoption itself. We validated this is not a meaningfully weaker detector than a semantic classifier: a stratified sample of 1,275 real commits (baseline, agentic, and this repo's own history) was scored independently by the regex and by an LLM judge (Jev, `typesafe/jev-1.13`, via the OpenRouter decisions endpoint); the two detectors agreed on all 1,275 (0 disagreements, cost ≈$0.006; raw data in `data/trials/`). No commit in that sample used AI tooling without also declaring it in the message — evidence that a semantic classifier adds nothing *for this sample*, not evidence that silent (undeclared) adoption never occurs elsewhere in the underlying repos. Silent adoption remains, by construction, undetectable from commit messages and is out of scope for this paper; every use of "adoption" or "declared attribution" throughout should be read as the latter.
